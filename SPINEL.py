...
def compute_strain(hkl, intensity, symmetry, lattice_params, wavelength, cij_params, sigma_params, chi, phi_values, psi_values):
    ...
    if isinstance(alpha_values, int):
        ...
        chi_rad = np.radians(chi)
        #alpha_grid = np.arctan2(np.sqrt(np.cos(theta0)**2*np.sin(delta_grid_rad)**2/(np.cos(chi_rad)**2*np.cos(theta0)**2*np.cos(delta_grid_rad)**2+np.sin(chi_rad)**2*np.sin(theta0)**2)),1)
        ...
        alpha_grid = np.sign(np.sign(delta_grid_rad)-1*np.cos(chi_rad)) * alpha_unsigned
    else:
