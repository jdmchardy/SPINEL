...
def compute_strain(hkl, intensity, symmetry, lattice_params, wavelength, cij_params, sigma_params, chi, phi_values, psi_values):
    ...
    if isinstance(psi_values, int):
        ...
        if sigma_11 == sigma_22 and sigma_12 == 0 and sigma_13 == 0 and sigma_23 == 0:
            ...
            new_psi_sampling = int(psi_sampling/2)
            ...
            phi_sampling = len(phi_values)
            new_sampling = int(phi_sampling/len(alpha_values))
            #Double the phi_sampling to compensate for reduction in psi sampling
            new_phi_sampling = 2*new_sampling
            st.write(new_phi_sampling)
            phi_values = np.asarray(np.radians(np.linspace(0,360, new_phi_sampling)))
            #Add deltas placeholder for completeness
            deltas = np.array([0])
            st.write("Total points:{}".format(new_psi_sampling*new_phi_sampling*36))
